...
const VolunteerCoordination = ({ disasterType, onClose, refreshTrigger }: VolunteerCoordinationProps) => {
  ...
  useEffect(() => {
    ...
    // Load volunteers from localStorage - only show those who passed the exam
    const users = getUsers();
    console.log("All users from localStorage:", users);
    
    const volunteersWithQualifications = users
      .filter(user => {
        const hasQualifications = !!user.qualifications;
        const examPassed = user.volunteerData?.examPassed === true;
        const isVolunteer = user.volunteerData?.isVolunteer === true;
        
        console.log(`\n--- User: ${user.fullName} ---`);
        console.log("Has qualifications:", hasQualifications, user.qualifications);
        console.log("Exam passed:", examPassed);
        console.log("Is volunteer:", isVolunteer);
        console.log("Volunteer data:", user.volunteerData);
        
        // Show ALL qualified volunteers, including the current user
        const isQualified = hasQualifications && examPassed && isVolunteer;
    ...
    console.log("Qualified volunteers:", volunteersWithQualifications);
  ...
  useEffect(() => {
    ...
    if (skillFilter !== "all") {
      filtered = filtered.filter(volunteer => {
        ...
        switch (skillFilter) {
          case "medical":
            return quals.hasFirstAid;
          case "transport":
            return quals.hasTransportation;
          case "experience":
            return quals.hasDisasterExperience;
          case "food":
            return quals.canProvideFood;
          case "shelter":
            return quals.canProvideAccommodation;
          default:
            return true;
        }
      });
      ...
      <div className="grid gap-4">
        ...
        {filteredVolunteers.length === 0 && volunteers.length === 0 ? (
          ...
          filteredVolunteers.map((volunteer) => {
            ...
            return (
              <Card key={volunteer.id} className={`border-l-4 ${isAvailable ? 'border-l-success bg-success/5' : 'border-l-warning bg-warning/5'}`}>
                <CardContent className="pt-6">

...
import { getCurrentUser, updateUserVolunteerData, setCurrentUser, getUsers, type VolunteerData } from "@/lib/localStorage";
...
const examQuestions: Question[] = [
  {
    id: 1,
    question: "What is the first priority during any emergency situation?",
    options: ["Save property", "Ensure personal safety", "Help others", "Call for help"],
    correct: 1
  },
  {
    id: 2,
    question: "In case of a flood, what should you NOT do?",
    options: ["Move to higher ground", "Walk through moving water", "Turn off utilities", "Have emergency supplies ready"],
    correct: 1
  },
  {
    id: 3,
    question: "What does the acronym STOP stand for in emergency response?",
    options: ["Stop, Think, Observe, Proceed", "Stay, Talk, Organize, Plan", "Secure, Transport, Organize, Protect", "Stop, Take cover, Observe, Plan"],
    correct: 0
  },
  {
    id: 4,
  ...
  },
  {
    ...
    options: ["2 short signals", "3 signals of any kind", "Continuous signaling", "1 long signal"],
    correct: 1
  },
  {
    id: 6,
    question: "In earthquake safety, what should you do if you're indoors?",
    options: ["Run outside immediately", "Stand in a doorway", "Drop, Cover, and Hold On", "Hide under stairs"],
    correct: 2
  },
  {
    id: 7,
  ...
  {
    id: 8,
    question: "What is the recommended emergency supply of water per person per day?",
    options: ["1 liter", "2 liters", "4 liters", "6 liters"],
    correct: 2
  },
  {
    id: 9,
    question: "What should be the first step when encountering a fire?",
    options: ["Try to extinguish it", "Alert others and evacuate", "Call 911", "Save valuable items"],
    correct: 1
  },
  {
    id: 10,
    question: "How often should emergency supplies be checked and updated?",
...
const VolunteerExam = ({ volunteerData, onExamComplete, onClose }: VolunteerExamProps) => {
  ...
  const calculateScore = (finalAnswers: number[]) => {
    ...
    const passed = scorePercentage >= 70; // 70% passing grade
    ...
    if (currentUser) {
      ...
      console.log("Saving volunteer data:", updatedVolunteerData);
      updateUserVolunteerData(currentUser.id, updatedVolunteerData);
      
      // Update the current user session with the latest data
      const users = getUsers();
      const updatedUser = users.find(u => u.id === currentUser.id);
      if (updatedUser) {
        console.log("Updated user found, refreshing session:", updatedUser);
      ...
      <Card className="w-full max-w-2xl mx-auto">
        ...
        <CardContent className="space-y-6">
          ...
          <div className="flex gap-4">
            {!passed && (
              <Button onClick={retakeExam} className="flex-1">
                Retake Exam
              </Button>
            )}
